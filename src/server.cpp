...
void Server::waitConnection()
{
    ...
    // TODO add more then 1 event in epoll result
    if (event.data.fd == listen_sd) {
        ...
        ConnectionNode connection_node =
            static_cast<ConnectionNode>(event.data.ptr);
        Connection& connection = connections.getConnection(connection_node);
        connection.setCanRead(event.events & EPOLLIN);
        connection.setCanWrite(event.events & EPOLLOUT);
...
void Server::returnConnectionToWaiters(Connection& connection)
{
    std::uint32_t new_events =
        connection.writeReady() ? EPOLLIN | EPOLLOUT : EPOLLIN;
...
void Server::addConnection()
{
    ...
    while ((socket = acceptConnection()) != 0) {
        ...
        epoll_event new_event = {.events = EPOLLIN,
                                 .data = {.ptr = connections.getBackNode()}};
